            		...
            		<div class="theme-page-wrapper mk-blog-single right-layout vc_row-fluid mk-grid ">
            			<div class="theme-content ">
            				<article id="506" class="post-506 post type-post status-publish format-standard has-post-thumbnail hentry category-health-2 category-lifestyle tag-branding tag-business-2 tag-design tag-marketing tag-photography-2">	
            					<h2 class="blog-single-title">${articles[0].title}</h2>
								<div class="blog-single-meta" style="float:right;">
								    <time class="mk-post-date">
								        ${articles[0].press} &nbsp;&nbsp;&#8227;&nbsp;&nbsp; ${articles[0].year}년&nbsp;&nbsp;${articles[0].month}월&nbsp;&nbsp;${articles[0].day}일
								    </time>
								</div>
								<div class="clearboth"></div>
								<c:if test="${!empty articles[0].images[0]}">
            					<div class="single-featured-image">
            						<div class="mk-swipe-slideshow" style="max-width:600px; margin:auto;">
            							<div data-loop="false" data-freemodefluid="false" data-slidesperview="1" data-pagination="false" data-freemode="false" data-mousewheelcontrol="false" data-direction="horizontal" data-slideshowspeed="7000" data-animationspeed="700" data-directionnav="false" class="mk-swiper-container mk-swiper-slider">
            								<div class="mk-swiper-wrapper">
            									 <c:forEach var="image" items="${articles[0].images}">
            									 <div class="swiper-slide">
            										 <img alt="" src="${image}" style="width: 100%;">
            									 </div>
            									 </c:forEach>
            								</div>
            								<c:if test="${!empty articles[0].images[1]}">
								    ...
								    <div class="wpb_row vc_row vc_row-fluid mk-fullwidth-false attched-false vc_row-fluid">
								        <div style="" class="vc_col-sm-12 wpb_column column_container ">
								            <div style=" margin-bottom:0px;text-align: left;" class="mk-text-block  ">
								                <p>${articles[0].contents}</p>
								                <div class="clearboth"></div>
								            </div>
								        </div>
								    </div>
								</div>
								<div class="single-post-tags">기사 출처 : <a href="${articles[0].url}" rel="tag" target="_blank" style="word-break:break-all;"> ${articles[0].url}</a></div>
							    ...
							    <div class="sidebar-wrapper">
							        <section class="widget widget_categories">
							            ...
							            <c:forEach begin="1" end="10" varStatus="i"> 
								            <c:if test="${!empty articles[i.count]}">
												<li style="white-space:nowrap;"><a href="javascript:;" onclick="jQuery('#article-form${i.count}').submit();">${ articles[i.count].title}</a></li>
							        ...
							        <c:if test="${!empty videos[0]}">
							        <section class="widget widget_categories">
							            <div class="widgettitle">관련 영상</div>
							            <ul> 
							            <c:forEach begin="1" end="10" varStatus="i"> 
								            <c:if test="${!empty videos[i.count-1]}">
												<li style="white-space:nowrap;"><a href="javascript:;" onclick="jQuery('#video-form${i.count-1}').submit();">${ videos[i.count-1].title}</a></li>												
												<form id="video-form${i.count-1}" accept-charset="utf-8" action="/Frontpage/video.do" method="post">
													<input type="hidden" name="id" value="${videos[i.count-1].id}"/>
													<input type="hidden" name="startYear" value="${section.startYear}"/>
													<input type="hidden" name="startMonth" value="${section.startMonth}"/>
													<input type="hidden" name="endYear" value="${section.endYear}"/>
													<input type="hidden" name="endMonth" value="${section.endMonth}"/>
							        ...
							        <section class="widget widget_categories">
							            ...
							            <ul> 
							            <c:forEach begin="1" end="10" varStatus="i"> 
								            <c:if test="${!empty articles[0].keywords[i.count-1]}">
								                <li style="white-space:nowrap;"><a href="javascript:;" onclick="jQuery('#search-form${i.count-1}').submit();">${ articles[0].keywords[i.count-1]}</a></li>
											 	<form id="search-form${i.count-1}" accept-charset="utf-8" action="/Frontpage/search.do" method="post">
											 		<input type="hidden" name="query" value="${articles[0].keywords[i.count-1]}">
